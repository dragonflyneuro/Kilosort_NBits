...
while ik<Nfilt
    ...
    for k = 1:50
        ...
        if (k>10 && rem(k,2)==1)
            ...
            StS_temp = StS(1:3,1:3);
            StMu_temp = StMu(1:3);
            w = StMu_temp'/StS_temp; % this is the new estimate of the best pursuit direection
            w = [normc(w');zeros(size(StMu,1)-3,1)]; % which we unit normalize
